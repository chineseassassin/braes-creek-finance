    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--color-bg-body)' }}>
      ...
      <div style={{ marginLeft: sidebarCollapsed ? 64 : 250, flex: 1, display: 'flex', flexDirection: 'column', transition: 'margin-left 0.2s ease' }}>
        ...
        <header style={{ height: 72, background: '#141414', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', position: 'sticky', top: 0, zIndex: 50, borderBottom: `1px solid ${COLORS.border}` }}>
          <div>
             <h1 style={{ fontSize: 20, fontWeight: 800, color: '#fff', margin: 0 }}>Inventory & Supply Control</h1>
             <p style={{ fontSize: 12, color: COLORS.muted, margin: 0 }}>Strategic stock management & predictive logistics</p>
          </div>
             ...
             <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                   ...
                   <div style={{ display: 'flex', gap: 64, marginBottom: 32 }}>
                      <div>
                         <div style={{ fontSize: 11, fontWeight: 700, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 4 }}>Total Inventory Value</div>
                         <div style={{ fontSize: 40, fontWeight: 950, color: '#fff' }}>${totalValue.toLocaleString()}</div>
                      </div>
                      <div>
                         <div style={{ fontSize: 11, fontWeight: 700, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 4 }}>Low Stock Items</div>
                         <div style={{ fontSize: 40, fontWeight: 950, color: lowStockCount > 0 ? COLORS.warning : '#fff' }}>{lowStockCount}</div>
                      </div>
                      <div>
                         <div style={{ fontSize: 11, fontWeight: 700, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 4 }}>Next Critical Shortage</div>
                         <div style={{ fontSize: 24, fontWeight: 900, color: '#fff', marginTop: 8 }}>{criticalItem ? criticalItem.name : 'None'}</div>
                      </div>
                   </div>
                ...
                <div style={{ display: 'flex', gap: 16, overflowX: 'auto', paddingBottom: 8 }}>
                   {hasData ? (
                      inventoryItems.filter(i => i.status === 'Critical' || i.status === 'Low').map((item, i) => (
                         <div key={i} className="card" style={{ minWidth: 280, padding: '20px', borderLeft: `4px solid ${item.status === 'Critical' ? COLORS.danger : COLORS.warning}` }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                               <span style={{ fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase' }}>{item.category}</span>
                      ...
                      <div style={{ width: '100%', height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.01)', borderRadius: 20, border: '1px dashed rgba(255,255,255,0.05)' }}>
                         <div style={{ fontSize: 13, color: COLORS.muted }}>Inventory alerts will activate once stock is added.</div>
                      </div>
                   )}
                </div>
             </div>
             ...
             <div className="col-8 card" style={{ padding: '32px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                   ...
                   <div style={{ display: 'flex', gap: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#141414', border: '1px solid #2a2a2a', borderRadius: 12, padding: '8px 16px' }}>
                         <Search size={14} color={COLORS.muted} />
                         <input 
                           placeholder="Search inventory..." 
                           value={searchTerm}
                           onChange={(e) => setSearchTerm(e.target.value)}
                           style={{ background: 'none', border: 'none', outline: 'none', color: '#fff', fontSize: 13, width: 180 }} 
                         />
                      </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                   <thead>
                      <tr style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                         <th style={{ padding: '16px', fontSize: 11, color: COLORS.muted, textTransform: 'uppercase' }}>Item Name</th>
                         <th style={{ padding: '16px', fontSize: 11, color: COLORS.muted, textTransform: 'uppercase' }}>Category</th>
                         <th style={{ padding: '16px', fontSize: 11, color: COLORS.muted, textTransform: 'uppercase' }}>Quantity</th>
                         <th style={{ padding: '16px', fontSize: 11, color: COLORS.muted, textTransform: 'uppercase' }}>Days Left</th>
                   ...
                   <tbody>
                      {filteredItems.map((item) => (
                         <tr key={item.id} className="row-hover" style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                            <td style={{ padding: '16px' }}>
                               <div style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>{item.name}</div>
                            </td>
                            <td style={{ padding: '16px' }}>
                               <span style={{ fontSize: 11, fontWeight: 700, color: COLORS.muted }}>{item.category.toUpperCase()}</span>
                            </td>
                            <td style={{ padding: '16px' }}>
                               <div style={{ fontSize: 13, fontWeight: 600, color: '#fff' }}>{item.qty} {item.unit}</div>
                               <div style={{ fontSize: 11, color: COLORS.muted }}>{item.usage}{item.unit}/week usage</div>
                            </td>
                      ...
                      {!hasData && (
                         <tr>
                            <td colSpan={6} style={{ textAlign: 'center', padding: '80px 0' }}>
                               ...
                               <div style={{ fontSize: 15, fontWeight: 700, color: '#fff', marginBottom: 4 }}>No inventory data yet</div>
                               <div style={{ fontSize: 13, color: COLORS.muted, marginBottom: 24 }}>Start by adding feed, fertilizer, or supplies to unlock monitoring.</div>
                         ...
                         return (
                            <div key={i}>
                               <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 8 }}>
                                  ...
                                  <span style={{ fontWeight: 900, color: '#fff' }}>${val.toLocaleString()} <span style={{ fontSize: 10, color: COLORS.muted, fontWeight: 500 }}>({((val / totalValue) * 100).toFixed(0)}%)</span></span>
                               </div>
                ...
                <div className="card" style={{ padding: '32px' }}>
                   ...
                   <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      {[
                         ...
                         <div key={i} style={{ padding: '16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 16, display: 'flex', gap: 12 }}>
                            <Sparkles size={16} color={insight.type === 'warning' ? COLORS.warning : (insight.type === 'danger' ? COLORS.danger : COLORS.info)} style={{ flexShrink: 0 }} />
                            <div style={{ fontSize: 13, color: '#fff', lineHeight: 1.4 }}>{insight.text}</div>
                         </div>
                      ))}
                   </div>
                </div>
        ...
        {isModalOpen && (
           <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
              <div className="card" style={{ width: 600, padding: '48px', position: 'relative' }}>
                 <button onClick={() => setIsModalOpen(false)} style={{ position: 'absolute', top: 24, right: 24, background: 'none', border: 'none', color: COLORS.muted, cursor: 'pointer' }}>
                    <X size={24} />
                 </button>
                 <h2 style={{ fontSize: 24, fontWeight: 900, color: '#fff', marginBottom: 32 }}>Add New Stock Item</h2>
                 
                 <div className="grid-12" style={{ gap: 20 }}>
                    <div className="col-12">
                       <label style={{ display: 'block', fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 8 }}>Item Name</label>
                       <input placeholder="e.g. Poultry Feed (Premium)" style={{ width: '100%', padding: '14px', background: '#141414', border: '1px solid #2a2a2a', borderRadius: 12, color: '#fff', fontSize: 14 }} />
                    </div>
                    <div className="col-6">
                       <label style={{ display: 'block', fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 8 }}>Category</label>
                       <select style={{ width: '100%', padding: '14px', background: '#141414', border: '1px solid #2a2a2a', borderRadius: 12, color: '#fff', fontSize: 14 }}>
                          {categories.map(cat => <option key={cat}>{cat}</option>)}
                       </select>
                    </div>
                    <div className="col-3">
                       <label style={{ display: 'block', fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 8 }}>Quantity</label>
                       <input type="number" placeholder="500" style={{ width: '100%', padding: '14px', background: '#141414', border: '1px solid #2a2a2a', borderRadius: 12, color: '#fff', fontSize: 14 }} />
                    </div>
                    <div className="col-3">
                       <label style={{ display: 'block', fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 8 }}>Unit</label>
                       <input placeholder="kg/L/bag" style={{ width: '100%', padding: '14px', background: '#141414', border: '1px solid #2a2a2a', borderRadius: 12, color: '#fff', fontSize: 14 }} />
                    </div>
                    <div className="col-6">
                       <label style={{ display: 'block', fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 8 }}>Unit Cost ($)</label>
                       <input type="number" placeholder="12.50" style={{ width: '100%', padding: '14px', background: '#141414', border: '1px solid #2a2a2a', borderRadius: 12, color: '#fff', fontSize: 14 }} />
                    </div>
                    <div className="col-12">
                       <label style={{ display: 'block', fontSize: 11, fontWeight: 800, color: COLORS.muted, textTransform: 'uppercase', marginBottom: 8 }}>Notes</label>
                       <textarea placeholder="Vendor details or storage location..." rows={3} style={{ width: '100%', padding: '14px', background: '#141414', border: '1px solid #2a2a2a', borderRadius: 12, color: '#fff', fontSize: 14, resize: 'none' }} />
                    </div>
