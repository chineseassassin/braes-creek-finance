...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Successful login -> trigger cinematic animation sequence
  ...
  return (
    <div className="min-h-[100dvh] bg-black flex flex-col items-center pt-[8vh] pb-12 px-4 font-inter relative overflow-y-auto overflow-x-hidden">
      <style>{`
        @keyframes scan {
          0% { top: -10%; opacity: 0; }
          15% { opacity: 1; }
      ...
      <div className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] blur-[150px] rounded-full pointer-events-none transition-colors duration-700 ease-in-out ${getGlowStyle()}`} />

      {/* Main Layout Container */}
      <div className="w-full max-w-[430px] relative z-10 flex flex-col items-center">
        
        {/* Brand Header */}
        <div className="flex flex-col items-center w-full" style={{ marginBottom: '40px' }}>
          <img 
            src="/bc-logo.png" 
            alt="Braes Creek Estate" 
            className="w-[240px] object-contain mb-6 drop-shadow-[0_0_20px_rgba(255,255,255,0.1)]" 
          />
        ...
        {/* Login Card - Glassmorphism Navy */}
        <div className="w-full min-h-[500px] bg-[#0B1120]/80 backdrop-blur-xl border border-white/10 rounded-[32px] px-[40px] py-[48px] shadow-[0_0_50px_rgba(0,0,0,0.8)] relative overflow-hidden flex flex-col gap-4">
          ...
          <form onSubmit={handleLogin} className="w-full flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full h-12 bg-black/50 border border-white/10 rounded-xl pl-4 pr-12 text-sm text-white placeholder-white/30 focus:outline-none focus:border-cyan-400 transition-all tracking-wider"
                    placeholder="••••••••"
                  />
                  <button 
                    type="button" 
            ...
            <button 
              type="submit"
              disabled={loginState !== 'idle'}
              className="w-full h-12 bg-[#06b6d4] text-black font-black rounded-xl text-xs uppercase tracking-[0.2em] shadow-[0_0_20px_rgba(6,182,212,0.4)] hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-100 disabled:scale-100 relative overflow-hidden flex items-center justify-center mt-2"
            >
          ...
          <button 
            ...
            disabled={loginState !== 'idle'}
            className="w-full h-12 bg-transparent border border-white/20 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-3 hover:bg-white/5 transition-colors disabled:opacity-50"
          >
